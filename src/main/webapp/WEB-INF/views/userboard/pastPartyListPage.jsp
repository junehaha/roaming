...
<script>
		...
		xhr.open("post", "../post/postReviewInsert");
		xhr.send(formData);
	}
            ...
            <div class="row">
                  ...
                  <div class="row mt-4">
                     <div class="col-3">
                        ...
                        <div class="row mt-4">
                           ...
                           <a href="./pastPartyListPage" style="text-decoration: none;"><p style="font-size: 15px; color: #0078ff;">지난 동행</p></a>
                        </div> 
                        <div class="row mt-4">
                           <h5>숙소</h5>
                                    ...
                                    <div class="col border" style="height: 210px; border-radius: 7px;">
                                        <div class="row mt-3">
                                            <div class="col-4" style="padding-left: 16px; ">
                                                <div class="border text-center" style="width: 70px; border-radius: 7px; font-size: 14px;">여행완료</div>
                                            </div>
                                            <div class="col-6">
                                        ...
                                        <div class="row mt-2">
                                            ...
                                            <div class="col-6" >
                                                ...
                                                <div class="row mt-1">
                                                    <div class="col p-0">
                                                        <p style="margin: 0 5px; font-size: 16px;">${list.postDto.content}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-2 text-end mt-2">
                                                <div class="dropdown" style="justify-content: center; display: flex;">
                                                    <c:if test="${list.partyUserList != null and (list.reviewCount == null or list.partyCount - 1 > list.reviewCount)}">
                                                    <a class="btn btn-outline-primary dropdown" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                        후기작성
                                                    </a>
                                                    </c:if>
                                                    <ul class="dropdown-menu" style="width: 250px;">
                                                    	<c:forEach items="${list.partyUserList}" var="item">
                                                        <li><a class="dropdown-item" href="#">
                                                        <div class="row mt-2 pb-2">
                                                            <div class="col-9">
                                                                <img src="/uploadFiles/${item.joinDto.profile_img}" alt="" style="width: 30px; height: 30px; border-radius: 50%;">
                                                                <span style="margin-left: 8px;">${item.joinDto.user_name}</span>
                                                            </div>
                                                            <div class="col-3 p-0">
                                                                <input class="btn btn-outline-primary p-0 " type="button" value="작성" 
                                                                onclick="reviewModal(${list.postDto.id}, '${list.postDto.title}', ${sessionUser.id}, '${sessionUser.user_name}', ${item.joinDto.id}, '${item.joinDto.user_name}');" 
                                                                style=" width: 50px; font-size: 15px; padding: 5px 0; margin-top: 3px;">
                                                            </div>
                                                        </div>
                                                        </a></li>
                                                        </c:forEach>
               ...
               <div class="modal fade" id="reviewModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		        <div class="modal-dialog">
		            ...
		            <div class="modal-header" style="height: 30px; position: relative;">
		                <h1 class="modal-title fs-5" style="position: absolute; left: 50%; transform: translateX(-50%);" id="exampleModalLabel">후기 쓰기</h1>
		                <button type="button" class="btn-close" onclick="reviewClose();"></button>
		            </div>
		            <div class="modal-body" style="overflow-y: auto; padding: 12px;">
		                <div class="row ">
		                    <div class="col">
		                        <div class="row">
		                            <div class="col">
		                                <span>동행제목 :</span>
		                                <span id="titleBox" style="font-size: 15px">서울가즈아</span>
		                            </div>
		                        </div>
		                        <div class="row mt-1">
		                            <div class="col">
		                                <span>작성자명 :</span>
		                                <span id="nameBox" style="font-size: 15px">김영수</span>
		                            </div>
		                        </div>
		                    </div>
		                </div>
		                <div class="row mt-3 mb-3 border-top border-bottom">
		                    <div class="col text-center" style="padding: 15px;">
		                        <p id="receiveBox" style="margin-bottom: 10px;">님과 여행은 어떠셨나요?</p>
		                        <div class="star-rating space-x-4 mx-auto" >
		                            <input type="radio" onclick="starRating(5);" id="5-stars" name="rating" value="5" v-model="ratings"/>
		                            <label for="5-stars" class="star">★</label>
		                            <input type="radio" onclick="starRating(4);" id="4-stars" name="rating" value="4" v-model="ratings"/>
		                            <label for="4-stars" class="star">★</label>
		                            <input type="radio" onclick="starRating(3);" id="3-stars" name="rating" value="3" v-model="ratings"/>
		                            <label for="3-stars" class="star">★</label>
		                            <input type="radio" onclick="starRating(2);" id="2-stars" name="rating" value="2" v-model="ratings"/>
		                            <label for="2-stars" class="star">★</label>
		                            <input type="radio" onclick="starRating(1);" id="1-star" name="rating" value="1" v-model="ratings" />
		                            <label for="1-star" class="star">★</label>
		                        </div>
		                        <span id="ratingBox">0 / 5</span>
		                    </div>
		                </div>
		                <div class="row mt-3">
		                    <div class="col">
		                        <textarea name="content" id="content" cols="" rows="6" class="form-control" placeholder="후기를 작성해주세요."></textarea>
		                    </div>
		                </div>
		                <div class="row mt-3 p-0">
		                    <div class="col d-grid" style="padding-right: 4px;">
		                        <button type="button" onclick="reviewClose();" class="btn btn-outline-secondary">취소</button>
		                    </div>
		                    <div class="col d-grid" style="padding-left: 4px;">
		                        <button type="button" onclick="reviewSubmit()" class="btn btn-outline-success">등록</button>
		                    </div>
		                </div>
		            </div>
		        </div>
