...
const xhr = new XMLHttpRequest();
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState==4 && xhr.status ==200){
			...
			const pimg = response.kakao_account.profile.profile_image_url;
			loginOrSign(kid,name,gender,pimg)
		}
	}
...
function loginOrSign(kid,name,gender,pimg){
const xhr = new XMLHttpRequest();
	...
	xhr.open("get","./loginOrSign?kid="+kid+"&name="+name+"&gender="+gender+"&pimg="+pimg);
	xhr.send()
}

function login(){
	window.location.href ="../userboard/mainPage";
}

function register(){
	window.location.href ="./kakaoRegister";
}
...
function kakaoLogin1(){
	window.location.href = "https://kauth.kakao.com/oauth/authorize?client_id=9c133e8e0ca99f4e08b962a90d53f8a4&redirect_uri=http://localhost:8181/roaming/user/loginPage&response_type=code"
}
</script>
</head>
